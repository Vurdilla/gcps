    ...
    vc3_phys::gpu_particles* gpuparticles)
{
    ...
    {
        v = gpuparticles->particle_chiT[poffset] * gpuparticles->particle_GSC[poffset] / (gpuparticles->particle_SC[poffset] + gpuparticles->particle_c0[poffset])
            + gpuparticles->particle_PPforceSpatialHashing[poffset]
            + gpuparticles->particle_V[poffset] * gpuparticles->particle_rot[poffset];
    }
    ...
    {
        v.normalize(gpuparticles->particle_V[poffset]);
    }
    ...
    if (__gpusetup_variables.chemosensitivityModel == 1) // gradient of log(SC)
    {
        omega = gpuparticles->particle_chiR[poffset] * gcostheta * sintheta / (gpuparticles->particle_SC[poffset] + gpuparticles->particle_c0[poffset]);
    }
    else if (__gpusetup_variables.chemosensitivityModel == 0) // gradient of SC
    {
    ...
    flt2 dphi = __gpusetup_variables.timeStep * (omega + uomega) + vc3_cumath::msqrt_flt2(2.00 * __gpusetup_variables.timeStep * gpuparticles->particle_DR[poffset]) * z1;
    ...
    if (nthit >= 0)
    {
        ...
        { // 0 - particle position to the center + random direction
            //resetToTheOrigin();
            gpuparticles->particle_flag_wasResetToCenter[poffset] = true;
    ...
    else if (__gpusetup_variables.boundaryType == 1 || __gpusetup_variables.boundaryType == 2) // Square arena
    {
    ...
    if (gpuparticles->particle_flag_boundaryHit[poffset])
    {
        ...
        { // direction to the center + one timestep towards center
            gpuparticles->particle_angle[poffset] = opos.phi() + vc3_cumath::Pi;
            sincos(gpuparticles->particle_angle[poffset], &(gpuparticles->particle_rot[poffset].y), &(gpuparticles->particle_rot[poffset].x));
            vc3_cumath::planar::cuvector dcpos = __gpusetup_variables.timeStep * gpuparticles->particle_V[poffset] * gpuparticles->particle_rot[poffset];
    ...
    if (gpuparticles->particle_flag_boundaryHit[poffset] && __gpusetup_variables.boundaryResetSRtime)
    {
        if (__gpusetup_variables.timedResetTimerType == 0)
        {
            ...
            if (__gpusetup_variables.globalResetTime >= 0.00 && __gpusetup_variables.globalResetTime > gpuvariables->currentTime &&
    ...
    // $ relaxBeta()
    // =============
    // 1. Normal relaxation to the default value
    // positive = there is a decay, zero = no decay, negative = reset to beta0 every timestep
    if (__gpuprecomputes.betaDecayRate > 0.00)
    { // positive: _deltaBeta *= _deltaBetaDecayRate;
    ...
    else if (__gpuprecomputes.betaDecayRate < 0.00)
    { // negative: reset to beta0 every timestep
        gpuparticles->particle_beta[poffset] = gpuparticles->particle_beta0[poffset];
    }
    ...
    if (__gpuprecomputes.VDecayRate > 0.00)
    { //_deltaV *= _deltaVDecayRate;
        gpuparticles->particle_V[poffset] = gpuparticles->particle_V0[poffset] +
            (gpuparticles->particle_V[poffset] - gpuparticles->particle_V0[poffset]) * __gpuprecomputes.VDecayRate;
    }
    else if (__gpuprecomputes.VDecayRate < 0.00)
    {
        gpuparticles->particle_V[poffset] = gpuparticles->particle_V0[poffset];
    }

    // 2. Reset to default value after boundary hit
    if (gpuparticles->particle_flag_boundaryHit[poffset])
    {
        if (__gpusetup_variables.boundaryResetType == 0)
        { // 0 - position to the center + random direction
        ...
        { // 1 - direction to the center
            ...
            gpuparticles->particle_V[poffset] = __gpusetup_variables.VBoundary;
        }
        else if (__gpusetup_variables.boundaryResetType == 2)
        { // 2 - reverse direction + one timestep backward
            gpuparticles->particle_beta[poffset] = gpuparticles->particle_beta0[poffset] * __gpusetup_variables.betaBoundaryMult;
    ...
    {
        int targetResetType = gputargets->target_resetType[gpuparticles->particle_flag_targetHit[poffset]];
        if (targetResetType == 0)
        { // 0 - position to the center + random direction
            gpuparticles->particle_beta[poffset] = gpuparticles->particle_beta0[poffset];
            gpuparticles->particle_V[poffset] = gpuparticles->particle_V0[poffset];
        ...
        else if (targetResetType == 1)
        { // 1 - direction to the center
            gpuparticles->particle_beta[poffset] = gpuparticles->particle_beta0[poffset] * 
                gputargets->target_betaMult[gpuparticles->particle_flag_targetHit[poffset]];
            if (gputargets->target_V[gpuparticles->particle_flag_targetHit[poffset]] >= 0.00)
                gpuparticles->particle_V[poffset] = gputargets->target_V[gpuparticles->particle_flag_targetHit[poffset]];
            //printf("relax beta: step=%lld, time=%.15le, pID=%d, beta=%.8le\n", gpuvariables->currentStep, gpuvariables->currentTime, poffset, gpuparticles->particle_beta[poffset]);
        }
        else if (targetResetType == 2)
        { // 2 - reverse direction + revert position one timestep backward
            gpuparticles->particle_beta[poffset] = gpuparticles->particle_beta0[poffset] *
                gputargets->target_betaMult[gpuparticles->particle_flag_targetHit[poffset]];
            if (gputargets->target_V[gpuparticles->particle_flag_targetHit[poffset]] >= 0.00)
    ...
    if (gpuparticles->particle_flag_timedReset[poffset])
    {
        gpuparticles->particle_beta[poffset] = gpuparticles->particle_beta0[poffset];
        gpuparticles->particle_V[poffset] = gpuparticles->particle_V0[poffset];
...
/** Device kernel (PBC Version)
    ...
    MODIFICATIONS:
    1. Gradient Calculation: Uses explicit index wrapping for bicubic interpolation
       to handle periodic boundaries.
**/
__global__ void __kernel_moveParticles_PBC(vc3_phys::gpu_variables* gpuvariables,
    vc3_phys::gpu_targets* gputargets, const vc3_phys::gpu_matrixes* __restrict__ gpumatrixes,
    vc3_phys::gpu_particles* gpuparticles)
{
    int poffset = blockIdx.x * blockDim.x + threadIdx.x;
    ...
    // ! NOTE: these calculations are hard-binded to the SC offset style!
    flt2 ay[4], by[4], cy[4], dy[4];
    long long int offsetm1m1 = vc3_phys::get_SCoffset((gpuparticles->particle_posLattice[poffset].x - 1 + __gpusetup_variables.latticeSize) % __gpusetup_variables.latticeSize, gpuparticles->particle_posLattice[poffset].y - 1, __gpusetup_variables.latticeSize);
    long long int offset0m1 = vc3_phys::get_SCoffset(gpuparticles->particle_posLattice[poffset].x, gpuparticles->particle_posLattice[poffset].y - 1, __gpusetup_variables.latticeSize);
    long long int offset1m1 = vc3_phys::get_SCoffset((gpuparticles->particle_posLattice[poffset].x + 1) % __gpusetup_variables.latticeSize, gpuparticles->particle_posLattice[poffset].y - 1, __gpusetup_variables.latticeSize);
    long long int offset2m1 = vc3_phys::get_SCoffset((gpuparticles->particle_posLattice[poffset].x + 2) % __gpusetup_variables.latticeSize, gpuparticles->particle_posLattice[poffset].y - 1, __gpusetup_variables.latticeSize);
    for (int y = 0; y < 4; y++)
    {
        // Calculate wrapped Y coordinate (handling PBC in Y direction)
        int wrapped_y = (gpuparticles->particle_posLattice[poffset].y - 1 + y + __gpusetup_variables.latticeSize) % __gpusetup_variables.latticeSize;

        // Calculate linear indices
        long long int offsetm1m1y = offsetm1m1 - (gpuparticles->particle_posLattice[poffset].y - 1) + wrapped_y;
        long long int offset0m1y = offset0m1 - (gpuparticles->particle_posLattice[poffset].y - 1) + wrapped_y;
        long long int offset1m1y = offset1m1 - (gpuparticles->particle_posLattice[poffset].y - 1) + wrapped_y;
        long long int offset2m1y = offset2m1 - (gpuparticles->particle_posLattice[poffset].y - 1) + wrapped_y;

        ay[y] = gpumatrixes->SC[offset0m1y];
        cy[y] = (gpumatrixes->SC[offset1m1y] + gpumatrixes->SC[offsetm1m1y] - 2.00 * gpumatrixes->SC[offset0m1y]) * 0.50;
        dy[y] = (3.00 * (gpumatrixes->SC[offset0m1y] - gpumatrixes->SC[offset1m1y])
            + gpumatrixes->SC[offset2m1y] - gpumatrixes->SC[offsetm1m1y]) / 6.00;
        by[y] = gpumatrixes->SC[offset1m1y] - ay[y] - cy[y] - dy[y];
    }
    ...
    gpuparticles->particle_dpos[poffset] = __gpusetup_variables.timeStep * (v + u);
    gpuparticles->particle_pos[poffset] += gpuparticles->particle_dpos[poffset];
    // MODIFICATION FOR PBC: Wrap coordinates
    if (gpuparticles->particle_pos[poffset].x < 0.00) gpuparticles->particle_pos[poffset].x += __gpusetup_variables.boxSize;
    else if (gpuparticles->particle_pos[poffset].x >= __gpusetup_variables.boxSize) gpuparticles->particle_pos[poffset].x -= __gpusetup_variables.boxSize;
    if (gpuparticles->particle_pos[poffset].y < 0.00) gpuparticles->particle_pos[poffset].y += __gpusetup_variables.boxSize;
    else if (gpuparticles->particle_pos[poffset].y >= __gpusetup_variables.boxSize) gpuparticles->particle_pos[poffset].y -= __gpusetup_variables.boxSize;
    // Update lattice position based on wrapped coordinates
    gpuparticles->particle_posLattice[poffset].x = gpuparticles->particle_pos[poffset].x * __gpuprecomputes.invdl;
    gpuparticles->particle_posLattice[poffset].y = gpuparticles->particle_pos[poffset].y * __gpuprecomputes.invdl;
    if (isnan(gpuparticles->particle_pos[poffset].x) || isnan(gpuparticles->particle_pos[poffset].y))
    {
    ...
    for (int nt = 0; nt < __gpusetup_variables.nTargets; nt++)
    {
        if (gputargets->target_active[nt])
        {
            vc3_cumath::planar::cuvector dr = gpuparticles->particle_pos[poffset] - gputargets->target_pos[nt];
            // MODIFICATION FOR PBC: Apply Minimum Image Convention (MIC)
    ...
    // $ boundaryCheck()
    // =================
    // MODIFICATION FOR PBC: No boundary hit logic required.
    vc3_cumath::planar::cuvector opos(gpuparticles->particle_pos[poffset].x - __gpuprecomputes.halfBox,
        gpuparticles->particle_pos[poffset].y - __gpuprecomputes.halfBox);
    // 1. Check boundary hit
    gpuparticles->particle_flag_boundaryHit[poffset] = false;
