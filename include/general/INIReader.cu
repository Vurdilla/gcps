...
public:
    ...
    // Get an integer (long) value from INI file, returning default_value if
    // not found or not a valid integer (decimal "1234", "-1234", or hex "0x4d2").
    long long int GetLLInteger(std::string section, std::string name, long long int default_value);
...
long long int INIReader::GetLLInteger(std::string section, std::string name, long long int default_value)
{
    std::string valstr = Get(section, name, "");
    const char* value = valstr.c_str();
    char* end;
    // This parses "1234" (decimal) and also "0x4D2" (hex)
    long long int n = strtoll(value, &end, 0);
    return end > value ? n : default_value;
}
